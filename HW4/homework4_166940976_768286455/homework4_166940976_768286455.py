...
def train(trainX, trainY, weightsAndBiases, testX, testY):
    NUM_EPOCHS = 100
    trajectory = []
    _, (X_val, y_val), _ = setup_MNIST(has_validation=True, show_logs=False)
...
def setup_MNIST(classes=10, has_validation=False, show_logs=True):
    if show_logs:
...
def problem_4():
    ...
    if RUN_WITH_SMALLER_SAMPLE:
        SAMPLE_SIZE = 2500
        # Run again for a smaller set of points! (2500)
        train_samples = np.random.choice(len(trainX), size=SAMPLE_SIZE, replace=False)
        test_samples = np.random.choice(len(testX), size=SAMPLE_SIZE, replace=False)
